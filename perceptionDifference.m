function [kY kCb kCr] = perceptionDifference(a,b)
%% convert to YCbCr
a1 = rgb2ycbcr(a);
b1 = rgb2ycbcr(b);
...
% for each octet
for h = 1:8:(size(a1y,1)-7)
    for w = 1:8:(size(a1y,2)-7) 
        % extract blockA
        ay = a1y(h:(h+7),w:(w+7));
        ...
        % add differences
        [avgeY deY] = evaluate_block(ay,by,Y,DM);
        kY = deY ./ avgeY;
        kYs = kYs + kY;
    ...
    if(pr ~= round(h/size(a1y,1)*10))
        disp([num2str(round(h/size(a1y,1)*10)) '0%']);
        pr = round(h/size(a1y,1)*10);
    end
end
toc;
